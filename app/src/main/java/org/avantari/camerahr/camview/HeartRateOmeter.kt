package org.avantari.camerahr.camview

import android.content.Context
import android.graphics.Point
import android.hardware.Camera
import android.hardware.Camera.Parameters.FOCUS_MODE_INFINITY
...
import org.avantari.camerahr.camview.camera.CameraModule
import org.avantari.camerahr.camview.camera.CameraSupport
...
open class HeartRateOmeter {
    ...
    private val publishSubject: PublishSubject<Bpm>
    ...
    fun setFlashEnabled(enable: Boolean): HeartRateOmeter {
        this.isFlashEnabled = enable
        return this
    }

    fun setCameraType(cameraType: Int, ref: AppCompatActivity? = null): HeartRateOmeter {
...
//            ref?.brighterScreen()
    ...
    protected fun start() {
        ...
        context?.get()?.let {
            if (cameraType == FRONT_CAMERA) {
                cameraSupport = CameraModule.provideCameraSupport(context = it).open(1)
            } else if (cameraType == BACK_CAMERA) {
    ...
    private fun getScreenDimensions(): Dimension {
        ...
        if (Build.VERSION.SDK_INT in 14..16) {
            try {
                ...
                screenHeight =
                    android.view.Display::class.java.getMethod("getRawHeight")
    ...
    protected fun setCameraParameter(width: Int, height: Int) {
        ...
        val modes = cameraSupport?.parameters?.supportedFlashModes
        ...
        if (cameraType == FRONT_CAMERA) {
            ...
            if (isFlashEnabled) {
                if (modes?.any { it == Camera.Parameters.FLASH_MODE_TORCH } == true) {
                    parameters?.flashMode = Camera.Parameters.FLASH_MODE_TORCH
                }
            } else {
                if (modes?.any { it == Camera.Parameters.FLASH_MODE_OFF } == true) {
                    parameters?.flashMode = Camera.Parameters.FLASH_MODE_OFF
                }
            }
        }
        ...
        cameraSupport?.parameters?.focusMode = FOCUS_MODE_INFINITY
        /* var flash = true
    ...
    protected fun createCameraPreviewCallback2(): Camera.PreviewCallback {
        ...
        return object : Camera.PreviewCallback {
            ...
            override fun onPreviewFrame(data: ByteArray?, camera: Camera) {
                ...
                if (imageAverage == 0 || imageAverage < 180) {
                    PROCESSING.set(false)
                    fingerDetected = false
                    return
                }
